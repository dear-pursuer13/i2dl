...
class DataLoader:
    ...
    def __iter__(self):
        ...
        def dictionary_format(batches):
            dic_current = []
            dictionary = []
            for i, batch in enumerate(batches):
                for j in range(0, len(batch)):
                    dic_current.append(dict(data=batch[j]))
                    if j == len(batch) - 1:
                        dictionary.append(dic_current)
                        dic_current = []
        ...
        batches = dictionary_format(batches)
    ...
    def __len__(self):
        ...
        batch = [i for i in range(0, limit) if i % self.batch_size == 0]
        if(self.drop_last == False):
            length = len(batch)
        elif(self.drop_last == True and limit != batch[-1] + self.batch_size):
            length = len(batch) - 1
